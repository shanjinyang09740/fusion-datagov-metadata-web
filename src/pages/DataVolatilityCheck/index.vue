    ...
    <div class="header">
      ...
      <div class="query">
        <fu-form :inline="true" :model="form">
          <fu-form-item label="指标" label-width="96px">
          ...
          <fu-form-item label="显示类型" label-width="100px">
            <fu-select
              size="medium"
              placeholder="请选择"
              :fu-request="typeSelectRequest"
              v-model="form.area"
              fu-id="fu-select-type"
              clearable
            >
              <fu-option
...
export default {
  ...
  computed: {
    ...
    typeSelectOptions() {
      return this.$store.state.reqData["fu-select-type"] || [];
    },
    //表格数据
    fuTableData() {
  ...
  beforeMount() {},
  mounted() {
    this.bgqListData = [2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017];
  },
  beforeUpdate() {},
  updated() {
